...
const saveRuntimeConfig = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.ops.systemLog.runtimeConfig.saveSuccess'))
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to save runtime log config', err)
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.runtimeConfig.saveFailed'))
  } finally {
...
const resetRuntimeConfig = async () => {
  const ok = window.confirm(t('admin.ops.systemLog.runtimeConfig.rollbackConfirm'))
  if (!ok) return
  ...
  try {
    ...
    runtimeConfig.stacktrace_level = saved.stacktrace_level
    runtimeConfig.retention_days = saved.retention_days
    appStore.showSuccess(t('admin.ops.systemLog.runtimeConfig.rollbackSuccess'))
    await fetchHealth()
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to reset runtime log config', err)
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.runtimeConfig.rollbackFailed'))
  } finally {
    runtimeSaving.value = false
  }
}
...
const cleanupCurrentFilter = async () => {
  const ok = window.confirm(t('admin.ops.systemLog.actions.cleanupConfirm'))
  if (!ok) return
  try {
    const payload = {
    ...
    appStore.showSuccess(t('admin.ops.systemLog.actions.cleanupSuccess', { count: res.deleted || 0 }))
    page.value = 1
    await Promise.all([fetchLogs(), fetchHealth()])
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to cleanup logs', err)
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.actions.cleanupFailed'))
  }
}
...
<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-gray-900 dark:text-white">{{ $t('admin.ops.systemLog.title') }}</h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $t('admin.ops.systemLog.description') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ $t('admin.ops.systemLog.health.queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ $t('admin.ops.systemLog.health.written') }} {{ health.written_count }}</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{{ $t('admin.ops.systemLog.health.dropped') }} {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700 dark:bg-red-900/30 dark:text-red-300">{{ $t('admin.ops.systemLog.health.failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">{{ $t('admin.ops.systemLog.runtimeConfig.title') }}</div>
        <span v-if="runtimeLoading" class="text-xs text-gray-500">{{ $t('admin.ops.systemLog.runtimeConfig.loading') }}</span>
      </div>
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        <label class="text-xs text-gray-600 dark:text-gray-300">
          {{ $t('admin.ops.systemLog.runtimeConfig.level') }}
        ...
        <label class="text-xs text-gray-600 dark:text-gray-300">
          {{ $t('admin.ops.systemLog.runtimeConfig.stacktraceLevel') }}
          ...
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
              <label class="inline-flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                {{ $t('admin.ops.systemLog.runtimeConfig.caller') }}
              </label>
              <label class="inline-flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                <input v-model="runtimeConfig.enable_sampling" type="checkbox" />
                {{ $t('admin.ops.systemLog.runtimeConfig.sampling') }}
              </label>
            </div>
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
              <button type="button" class="btn btn-primary btn-sm" :disabled="runtimeSaving" @click="saveRuntimeConfig">
                {{ runtimeSaving ? $t('admin.ops.systemLog.runtimeConfig.saving') : $t('admin.ops.systemLog.runtimeConfig.save') }}
              </button>
              <button type="button" class="btn btn-secondary btn-sm" :disabled="runtimeSaving" @click="resetRuntimeConfig">
                {{ $t('admin.ops.systemLog.runtimeConfig.rollback') }}
              </button>
            </div>
          </div>
        </div>
      </div>
      <p v-if="health.last_error" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $t('admin.ops.systemLog.health.lastError') }}: {{ health.last_error }}</p>
    </div>

    <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-5">
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.timeRange') }}
      ...
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.component') }}
        <input v-model="filters.component" type="text" class="input mt-1" :placeholder="$t('admin.ops.systemLog.filters.componentPlaceholder')" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.requestId') }}
        <input v-model="filters.request_id" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.clientRequestId') }}
        <input v-model="filters.client_request_id" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.userId') }}
        <input v-model="filters.user_id" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.accountId') }}
        <input v-model="filters.account_id" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.platform') }}
        <input v-model="filters.platform" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.model') }}
        <input v-model="filters.model" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ $t('admin.ops.systemLog.filters.keyword') }}
        <input v-model="filters.q" type="text" class="input mt-1" :placeholder="$t('admin.ops.systemLog.filters.keywordPlaceholder')" />
      </label>
    </div>

    <div class="mb-3 flex flex-wrap gap-2">
      <button type="button" class="btn btn-primary btn-sm" @click="applyFilters">{{ $t('admin.ops.systemLog.actions.search') }}</button>
      <button type="button" class="btn btn-secondary btn-sm" @click="resetFilters">{{ $t('admin.ops.systemLog.actions.reset') }}</button>
      <button type="button" class="btn btn-danger btn-sm" @click="cleanupCurrentFilter">{{ $t('admin.ops.systemLog.actions.cleanup') }}</button>
      <button type="button" class="btn btn-secondary btn-sm" @click="fetchHealth">{{ $t('admin.ops.systemLog.actions.refreshHealth') }}</button>
    </div>
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-dark-700">
          <thead class="bg-gray-50 dark:bg-dark-900">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ $t('admin.ops.systemLog.table.time') }}</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ $t('admin.ops.systemLog.table.level') }}</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ $t('admin.ops.systemLog.table.details') }}</th>
            </tr>
          </thead>
